        ...
        {isMenuOpen && (
          ...
          >
            <nav className="flex flex-col items-center gap-6 pt-4">
              {NAV_LINKS.map((link) => (
                <motion.button key={link.id} variants={menuItemVariants} onClick={() => scrollToSection(link.id)} className="text-gray-600 hover:text-pink-400 transition-colors duration-300 font-medium text-lg">
                  {link.title}
                </motion.button>
              ))}
              <motion.button variants={menuItemVariants} onClick={() => scrollToSection('contacto')} className="bg-pink-400 text-white w-full px-5 py-3 rounded-full font-semibold hover:bg-pink-500 transition-transform duration-300 hover:scale-105 mt-4">
                Reservar Turno
              </motion.button>
            </nav>
          </motion.div>
